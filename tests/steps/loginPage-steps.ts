...
export class LoginPageSteps {
	...
	public async validateLoginLabels(): Promise<void> {
		await this.loginPage.validateUsernameLabel(
			loginPageLabelNames.usernameLabel,
		);
		await this.loginPage.validatePasswordLabel(
			loginPageLabelNames.passwordLabel,
		);
