...
export class BooksPage extends BasePage {
  ...
  private titleInput = () => this.page.locator("#title");
  private authorInput = () => this.page.locator("#author");
  ...
  public async enterPublicationDate(date: string): Promise<void> {
    await this.publicationDateInput().dblclick();
    await this.publicationDateInput().fill("");
  ...
  public async clickEditForBook(title: string): Promise<void> {
    ...
    log(`Book row found and ready for update '${title}'`, LogType.INFO);
  }
  ...
  public async clickDeleteForBook(title: string): Promise<void> {
    const bookRow = await this.searchRowForRequiredElement("tr", title);
    log(`Book row found and ready for deletion '${title}'`, LogType.INFO);
